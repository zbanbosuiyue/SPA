var CreateNewTransferController = function ($scope, $uibModalInstance, $rootScope, data, Upload, Api) {
	...
	$scope.addAlert = function(type, message) {
		if (type == 'danger'){
			$scope.alertStyle = {'background-color': 'red', color: 'white'};
		} else{
			$scope.alertStyle = {'background-color': 'green', color: 'white'};
		}
	...
	$scope.submit= function (){
		if ( $scope.skuIndex == null || $scope.requestQtyIndex == null){
			$scope.addAlert('danger', 'Error:  Not found "sku" or "request_qty" in file!!!');
			return;
		}
		...
		Api.PostApiCall("WarehouseTransfer", "CreateTransferRequestWithReturn", request1).then(function (createTransferEvent) {
		    ...
		    if (createTransferEvent.hasErrors == true) {
		    	...
		    	if(createTransferEvent.result.PkTransferId != null){
		    		$scope.fkStockItemIdArr.forEach(function(element, index){
		    			...
		    			Api.PostApiCall("WarehouseTransfer", "AddItemToTransfer", request2).then(function (addTransferItemEvent){
		    				if (addTransferItemEvent.hasErrors == true) {
		    					...
		    					Api.PostApiCall("WarehouseTransfer", "ChangeTransferItemRequestQuantity", request3).then(function(changeTransferItemEvent){
	...
	function LoadFileData(fileArray){
		$scope.bufferData = fileArray;
			...
			if (event.hasErrors == true) {
				...
				if(event.result.IsError == false){
					...
					if ($scope.bufferData.length != $scope.fkStockItemIdArr.length){
						...
						$scope.addAlert('danger', alertString);
						...
						$scope.addAlert('success', alertString);
					}
				}
			}
		});
